...
from .const import (
    ...
    DEFAULT_COMFORT_TEMP, MIN_COMFORT_TEMP, MAX_COMFORT_TEMP,
)
...
class SHAComfortTempNumber(NumberEntity, RestoreEntity):
    """Per-room comfort (standby) temperature — user-adjustable, restored across restarts.

    Set by user from the HA UI. Read by blueprint for standby heating target.
    Never updated by AI.
    """

    _attr_should_poll = False
    _attr_has_entity_name = True
    _attr_native_min_value = MIN_COMFORT_TEMP
    _attr_native_max_value = MAX_COMFORT_TEMP
    _attr_native_step = 0.5
    _attr_native_unit_of_measurement = "°C"
    _attr_mode = NumberMode.BOX
    ...
    async def async_set_native_value(self, value: float) -> None:
        ...
        self._value = round(value, 1)
        _LOGGER.debug(
            "[%s] Comfort temperature updated: %.1f → %.1f °C",
            self._room_name, old, self._value,
        )
        self.async_write_ha_state()
    ...
    async def async_added_to_hass(self) -> None:
        ...
        if (last := await self.async_get_last_state()) is not None:
            try:
                val = float(last.state)
                if MIN_COMFORT_TEMP <= val <= MAX_COMFORT_TEMP:
                    self._value = val
                    _LOGGER.debug(
                        "[%s] Comfort temperature restored: %.1f °C",
                        self._room_name, self._value,
                    )
                else:
                    _LOGGER.debug(
                        "[%s] Restored comfort temp %.1f out of range [%.1f, %.1f] — using default",
                        self._room_name, val, MIN_COMFORT_TEMP, MAX_COMFORT_TEMP,
                    )
            except (ValueError, TypeError):
                _LOGGER.debug(
                    "[%s] Could not restore comfort temp from state '%s' — using default",
                    self._room_name, last.state,
                )
        else:
            _LOGGER.debug("[%s] No previous comfort temp state — using default %.1f °C", self._room_name, self._value)

        registry = er.async_get(self.hass)
        expected_entity_id = f"number.sha_{self._room_id}_comfort_temp"
        current_entity_id = self.entity_id

        # Migrate from old default_temp entity_id if needed
        old_entity_id = f"number.sha_{self._room_id}_default_temp"
        old_entry = registry.async_get(old_entity_id)
        if old_entry and old_entry.unique_id == f"sha_{self._room_id}_default_temp":
            _LOGGER.warning(
                "SHA number: migrating entity_id %s → %s (default_temp → comfort_temp rename)",
                old_entity_id, expected_entity_id,
            )
            registry.async_update_entity(old_entity_id, new_entity_id=expected_entity_id)
